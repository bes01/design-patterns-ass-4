...
class SqlLiteRepository:
    ...
    def add_item(self, receipt_id: int, item_id: int, quantity: int) -> None:
        ...
        if self._receipt_item_exists(receipt_id, item_id):
            cursor.execute("update Receipt_Items set quantity = quantity + ? "
                           "where receipt_id = ? and item_id = ?", [quantity, receipt_id, item_id])
        else:
            cursor.execute("insert into Receipt_Items(receipt_id, item_id, quantity) values (?, ?, ?)",
    ...
    def close_receipt(self, receipt_id: int) -> None:
        if self._receipt_exists(receipt_id, 0):
            cursor = self._datasource.cursor()
            cursor.execute("update Receipts set closed = 1 "
                           "where _id = ?", [receipt_id])
            self._datasource.commit()
            cursor.close()
        else:
    ...
    def _receipt_exists(self, receipt_id: int, closed: int) -> bool:
        cursor = self._datasource.cursor()
        cursor.execute("SELECT EXISTS(SELECT 1 FROM Receipts WHERE _id= ? and closed = ?);", [receipt_id, closed])
        receipt_exists = cursor.fetchone()
        cursor.close()
    ...
    def get_current_day_receipts(self) -> List[Receipt]:
        cursor = self._datasource.cursor()
        cursor.execute("select _id from Receipts where open_date = ? and closed = 1", [datetime.date.today()])
